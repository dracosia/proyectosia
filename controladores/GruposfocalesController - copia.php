...
class GruposfocalesController extends BaseController {
	...
	*/
	...
	public function buscar($arrainput)
	{
		...
		$sql_uni="Select en.*, pro.nombre as programa, ind.nombre as indicadores, ind.id_codigo as codigo_indicador 
				 from gruposfocales en 
	...
	public function guardar($arrainput){
		...
		if($arrainput['codigo_id']=="" ||$arrainput['codigo_id']=="-1"){
			...
			$result=DB::insert("insert into gruposfocales set $campos");
			
		}else{
			// modificar o eliminar 
			
			$result=DB::update("update gruposfocales set $campos where codigo=".$arrainput['codigo_id']);
