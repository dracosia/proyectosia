...
class EntrevistasController extends BaseController {
...
/*		$sql_uni="Select en.*, pro.nombre as programa, ind.nombre as indicadores 
	...
	public function guardar($arrainput){
		...
		if($arrainput['codigo_id']=="" ||$arrainput['codigo_id']=="-1"){
			...
			$result=DB::insert("insert into entrevistas set $campos");
			
		}else{
			// modificar o eliminar 
			
			$result=DB::update("update entrevistas set $campos where codigo=".$arrainput['codigo_id']);
