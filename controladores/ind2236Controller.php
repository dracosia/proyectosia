...
class ind2236Controller extends BaseController {
...
/*		$sql_uni="Select en.*, pro.nombre as programa, ind.nombre as indicadores 
	...
	public function guardar($arrainput){
		...
		if($arrainput['tipo']!="eliminar"){
			...
			$campos="programa_fk=$programa, indicador_fk=$indicador, c1=$c1, c2=$c2, c5=$c5, estado_fk=$estado";
		...
		if($arrainput['codigo_id']=="" ||$arrainput['codigo_id']=="-1"){
			...
			$result=DB::insert("insert into ind2236 set $campos");
			
		}else{
			// modificar o eliminar 
			
			$result=DB::update("update ind2236 set $campos where codigo=".$arrainput['codigo_id']);
