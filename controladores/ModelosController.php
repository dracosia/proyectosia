...
class ModelosController extends BaseController {
	...
	*/
	...
	public function buscar($arrainput)
	{
		...
		$sql_uni="Select *  from modelos where  codigo is not null $cadena and estado_fk=1 and empresa_fk=$valorempresa ";
	...
	public function guardar($arrainput){
		...
		if($arrainput['codigo_id']==""){
			//insertar
			$cadena_mostrar="insert into modelos set $campos";
			$result=DB::insert("insert into modelos set $campos");
			
		}else{
			// modificar o eliminar 
			
			$result=DB::update("update modelos set $campos where codigo=".$arrainput['codigo_id']);
